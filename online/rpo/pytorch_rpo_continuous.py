...
class ActorCriticNet(nn.Module):
    def __init__(self, observation_shape, action_space, actor_layers, critic_layers, rpo_alpha, device):
        super().__init__()

        self.action_low = torch.from_numpy(action_space.low / 2).to(device)
        self.action_high = torch.from_numpy(action_space.high / 2).to(device)
    ...
    def evaluate(self, states, actions):
        ...
        mean = self.actor_mean(output) + Uniform(self.action_low, self.action_high).sample()
        std = torch.sigmoid(self.actor_std(output)) + 1e-7
...
def train(args, run_name, run_dir):
    ...
    # Create buffers
    rollout_buffer = RolloutBuffer(args.num_steps, args.num_envs, observation_shape, action_shape, args.device)
...
def eval_and_render(args, run_dir):
    ...
    # Load policy
    policy = ActorCriticNet(observation_shape, action_dim, args.list_layer, args.rpo_alpha, args.device)
...
if __name__ == "__main__":
    ...
    run_name = "RPO_PyTorch"
    run_dir = f"logs/{args_.env_id}__{run_name}__{run_time}"
